...
represents the assignment of a few Smart Home features to 
...
Register Inhabitant scenario has seven steps (\emph{A1, A2, \ldots, A7}). Two of
these steps were assigned only to the \emph{Register Inhabitant} (RI) feature; other two steps
...
the \emph{structured data operation} features is scattered through the scenarios
\emph{Receive a MMS} (RM) and \emph{Select a MMS for Displaying} (SM). This means
that, the same specification of \emph{Store an Embedded Number} and \emph{Send
Message to Embedded Email} features changes the behavior of the RM and SM
features in a similar way. For this reason, we classify the behavior of the
\emph{Store an Embedded Number} and \emph{Send Message to Embedded Email}
features as \emph{homogeneous}. On the other hand, we say that the
\emph{Password} optional feature of the Smart Home case study has a
\emph{heterogeneous crosscutting behavior}, since its specification could not be
modularized in a unique advice. The same occured with the \emph{Fingerprint}
feature.
...
consider as being homogeneous or heterogenous, has a great influence in our
results. For instance, Table~\ref{tab:feature-dos} shows the DOS of
the features depicted in Figures~\ref{fig:smartHomeScenarios}
and~\ref{fig:MMSScenarios}. Note that, in MSVCM, modularizing the behavior
...
the DoF metric. As shown in Table~\ref{tab:scenario-dof}, base scenarios in
MSVCM have a higher degree of focus. In the remainder of this section we
present the sumarized data from the aforementioned case studies. 
...
\begin{tabular}{llll} \hline
							&				& PLUSS	& MSVCM	\\ \hline \\
\multirow{2}{*}{Smart Home} &	SC01		& 0.24  & 1.00  \\
							&	SC02		& 0.27  & 1.00  \\ 
\multirow{2}{*}{MMS} 		&	SC01		& 0.12  & 1.00  \\ 
							&	SC02		& 0.20  & 1.00  \\ 
\end{tabular}
\end{small}
\end{table}
