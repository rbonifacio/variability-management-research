...
semantics of the \emph{weaving process}~\ref{fig:weave-process}. The weving
process is responsible for generating a use case model that is specific to the 
product configuration. Therefore, the process takes into accout a selection of
...
weavingProcess spl fm pc ck = 
 stepRefinement [(w spl) | w <- ws] product
 where 
  ws = concat [weavers c| c <- ck, eval pc (exp c)]
  product = (emptyInstance spl fc)
  stepRefinement l m = ...   	
\end{code}

Each weaver is a function that has the signature
SPL Product Instance ProductInstance. 
