...
\begin{figure*}[hbt] 
 \begin{code}
  (|->) :: a -> b -> (a,b) -- just a syntactic function to build pairs
  l |-> r = (l,r)
...
Figure~\ref{fig:ck-running-example} presents a configuration knowledge for the
running example, enforcing that the \emph{Proceed to Purchase} and \emph{Search
for Products} are mandatory scenarios, since they are related to the root feature
of \emph{eShop} product line; the \emph{Buy a Product} advice is used in the
composition of products that do not have been configured with both \emph{Shopping
Cart} and \emph{Bonus} features --- if both features were selected for a product,
it will be configured with the \emph{Buy Product with Cart} advice; and
finally, Figure~\ref{fig:ck-running-example} states that \emph{Register User
Preference} advice is not used in composition unless the \emph{Update
User Preferences } feature has been selected. 
...
proposed by Baachmann et al.~\cite{Bachmann:2001aa}, the first weave deals with
