\begin{code}
module UseCaseModel.Transformations (selectScenarios)
where
...
selectScenarios :: [Id] -> FeatureConfiguration -> UseCaseModel -> UseCaseModel -> UseCaseModel
selectScenarios ids fc spl product = addScenarios scenarios spl product
 where scenarios = [s | s <- splScenarios spl, scId s `elem` ids]
  
bindParameter :: Id -> Id -> FeatureConfiguration -> UseCaseModel -> UseCaseModel -> UseCaseModel
bindParameter pId fId fc spl product = 
 bindParameter' product steps
 where 
   options = selectedOptions (fId, fc)
   steps = [bind s options| ucmSteps product, s `refers` ]
   bindParameter' p scs = 

selectedOptions (i, fc) = 
 let ftree = [x | x <- flatten fc, fId x == i]
 in case ftree of 
  []  -> error "Feature not fount"
  fs  -> featureOptionsValues (head fs)

addScenarios :: [Scenario] -> UseCaseModel -> UseCaseModel -> UseCaseModel
addScenarios [] spl product = product
addScenarios (x:xs) spl product = 
 addScenarios xs spl (addScenario x spl product)

addScenario :: Scenario -> UseCaseModel -> UseCaseModel -> UseCaseModel
addScenario s spl product = replaceUseCaseInUCM product (replaceScenarioInUseCase  uc s)
  where 
   uc = if (isJust (findUseCaseFromScenario (useCases product) s)) 
          then fromJust (findUseCaseFromScenario (useCases product) s)
          else emptyUseCase (findUseCaseFromScenario (useCases spl) s)
