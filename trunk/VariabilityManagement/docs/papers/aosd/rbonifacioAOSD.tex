...
semantics of the \emph{weaving process}~\ref{fig:weave-process}. The weaving
process is responsible for generating a use case model that is specific to the 
...
represented as a sequence of the algebraic data type $(exp,\ weavers)$, and a valid
interpreter for the weaving process. Basically, 
the interpreter evaluates a list of weavers ($ws$) that should be applied for
the specific product configuration. In order to filter the list of weavers, it
is necessary to verify  ($eval\ pc\ (exp\ c)$) which expressions ($exp\ c$)
are valid for a specific product configuration ($pc$).
...
The $stepRefinement$ function composes the sequence of weavers that should
be apploed. Therefore, supposing $ws = [h,g,f]$, the semantics of
the final product would be given by:

\begin{center}
$ p\ =\ f \bullet g \bullet h (emptyProduct)  $
\end{center}
