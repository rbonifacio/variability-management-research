...
On the other hand, we claim that our modeling framework is able to represent
...
scenario corresponds to textual use case scenarios. We explain the structure
...
{\color{blue}In our modeling framework, the concrete semantics of the weaving process (and the
...
Notice that instances of the input models are specified using  \emph{third-part} tools, and
...
this kind of verification. Basically, these functions verify if all constraints defined in
...
applied for each type of relationship (mandatory, alternative,
or). Moreover, additional functions were defined for importing feature models,
generated by the \emph{Feature Modeling Plugin}~\cite{}, to our environment.} 

It is important to note that we are not
proposing a new notation for feature modeling. Here we only present the features
required (Figure~\ref{fig:eshop-fm}) for understanding the MSVCM example. For
...
are also supported in PLUSS and PLUC, but, again, they are directly related to features 
or members of the product line.}

{\color{red} Additionally, the \mbox{{\bf [RegisterPreference]}} annotation (Step P3) is another variation point of \emph{Proceed to Purchase}. This annotation, also applied in the Step S3 of \emph{Search for Products} scenario~\ref{}, is used to reveal points in the specification that are related to the user preferences. Pointcuts can make reference to this annotations. For instance, the advice \emph{Update User Preferences}~\ref{} can extend the behavior of \emph{Proceed to Purchase} from Step P3. Note that our annotations were proposed just to expose variation points. Therefore, 
annotated steps are \emph{obliviousness} about which features are interested in extend the corresponding behavior. }
...
{\color{red} The \emph{pointcut} clause can refer either to step ids or step annotations. However, we encourage definition 
of pointcuts based on annotations, since they were proposed to mitigate the problem of fragile pointcuts. 
This problem occurs because simple changes in the order of steps can break the related pointcuts that refer to step ids. This 
problem might also be solved by means of specialized editors.}
...
which performs a search based on the input criteria. The same annotation was assigned to
...
generating a specific product. We provide the abstract syntax of this artifact in
the appendix. Therefore, the configuration knowledge allows, during product
engineering, the automatic configuration of SPL assets. Table~\ref{tab:eshop-ck}
