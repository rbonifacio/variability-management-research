...
\texttt{checkConfiguration} takes as input a feature model (FM) and a product
configuration (\emph{PC}), returning a list with all constraints that a
...
Presenting more details, the \texttt{checkConfiguration} function traverses both
feature model and product configuration trees, calling specific functions for checking the constraints
applied for each type of feature relationship (mandatory, exclusive or,
inclusive or) and constraints. Moreover, additional functions were defined for importing feature models,
generated by the \emph{Feature Modeling Plugin} (FMP)~\cite{Czarnecki:2004aa}, to our environment.} 

{\color{red}Although we are not
proposing a new notation for feature modeling, we had to create specific data types for feature models in Haskell.  The same was true for the other input models of our weaving process. For instance, bellow we show the feature model data type. It basically consists of a root feature and a list of global constraints. Due to lack of space, we do not present the data types of the other input models. 
...
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}\Varid{featureType}\mathbin{::}\Conid{FeatureType},\mbox{\onelinecomment  (optinal, mandatory)}{}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}\Varid{featureGroup}\mathbin{::}\Conid{Group},\mbox{\onelinecomment  (none, ior, eor)}{}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}\Varid{children}\mathbin{::}\Conid{Children}{}\<[E]%
\\
\>[B]{}\mskip1.5mu\}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
 
Finally, in the remainder of this section, we consider the eShop feature model introduced in the motivating example (Figure~\ref{fig:eshop-fm}). }
...
association joining this models and all relationships between them are kept by
...
{\color{red} Regarding to the tool support, instances of the use case model can be written in any text editor that is able to export documents using a specific XML format. However, aiming at simplifying the adoption of the proposed environment, and also to integrate it to the TaRGeT tool, we have proposed a Microsoft Word template that can be used for specifying use case models in the expected XML format. Additional functions were developed for parsing these documents to the abstract data type of our use case model.}  
...
Purchase} scenario for different configurations of \emph{Shipping Method}. {\color{red} Parameters 
...
{\color{red} The annotation \mbox{{\bf CustomerPreferences}} (Step P3) is another variation point of \emph{Proceed to Purchase} scenario. This annotation, also applied in the Step S3 of \emph{Search for Products} (Figura~\ref{fig:search-products-flow}), reveals points in the specification that are related to the customer preferences. Since pointcuts can make references to annotations, the advice \emph{Register User Preferences} (Figura~\ref{fig:register-preferences-flow}) extends the behavior of \emph{Proceed to Purchase} after Step P3. Note that our annotations just reveal variation points, which means that  
...
{\color{red} Hence, the \emph{pointcut} clause can refer either to step ids or step annotations (as explained before). However, we encourage definition 
...
{\bf Search for Products:} this mandatory scenario (Figure~\ref{fig:search-products-flow}) allows
a customer to search for products. In order to save space, we only present Step S3,
which performs a search based on the input criteria. {\color{red}Note that, similarly to the Step P3 shown in Figure~\ref{fig:proceed-to-checkout}, this step is also marked with the {\bf CustomerPreferences} annotation. Therefore, any advice that points to the CustomerPreferences annotation extends, at least, the behavior of \emph{Proceed to Purchase} and \emph{Search for Product} scenarios.}
...
{\color{red} At this point we are able to present some remarks related to our use case model. As discussed in Section~\ref{sec:problem}, PLUSS and PLUC represent all valid
configurations of a scenario in a single artifact. Differently, using our approach, we were
...
principle~\cite{Meyer:2000aa}; which means that, in our approach, introducing new product variants
...
aware of which scenarios are present in a specific product configuration.}
...
constraints).
 ...
 {\color{red}Similarly to the feature modeling, we use the FMP tool 
...
In our approach, the features selected to a specific product (represented as a product
configuration) identify which \emph{tasks} (or transformations) must be performed to
generate the corresponding SPL member. The relationships between features and 
configuration tasks are represented in the configuration knowledge, which is detailed in the 
next section. 
...
feature expressions to tasks used for generating a specific product. Therefore, the configuration knowledge allows, during product engineering, the automatic configuration of SPL assets. 
We provide the abstract syntax of this artifact in the appendix. 

Feature expressions have to be written in propositional logic, because we need to express, for example, that a task must be applied iff one configuration of features is satisfied by a product. For instance, the advice Buy Products is only evaluated iff the feature expression {\bf not ShoppingCart and Bonus} is true for a specific product configuration.}


Table~\ref{tab:eshop-ck}
presents the configuration knowledge for the running example, enforcing that
...
The model of this weaver is in Table~\ref{tab:sc-weaver}. The output is a new configuration of the \emph{product specific use case model}, whose scenarios were extended by alternative (or optional) flows of events. The extensions occur before or after the steps that match ($O_{JP}$) the pointcut clause ($UCM_{ID}$) of the advice, declared in the SPL use case model (UCM).
...
plays a fundamental role in this process, retrieving the steps in the use case
model that satisfy the pointcut clauses.
...
\\
\>[B]{}\hsindent{5}{}\<[5]%
\>[5]{}\Varid{stepRefinement}\;[\mskip1.5mu (\Varid{t}\;\Varid{spl})\mid \Varid{t}\leftarrow \Varid{ts}\mskip1.5mu]\;\Varid{empty}\;\mathbf{where}{}\<[E]%
\\
\>[5]{}\hsindent{1}{}\<[6]%
...
\\
\>[5]{}\hsindent{1}{}\<[6]%
\>[6]{}\Varid{empty}\mathrel{=}(\Varid{emptyInstance}\;\Varid{spl}{}\<[E]%
\\
\>[5]{}\hsindent{1}{}\<[6]%
\>[6]{}\Varid{fc}){}\<[E]%
\\
