...
{\color{red} The \mbox{{\bf RegisterPreference}} annotation (Step P3) is another variation point of \emph{Proceed to Purchase}. This annotation, also applied in the Step S3 of \emph{Search for Products} scenario~\ref{}, is used to reveal points in the specification that are related to the user preferences. Pointcuts can make reference to annotations. For instance, the advice \emph{Update User Preferences}~\ref{} extends the behavior of \emph{Proceed to Purchase} after Step P3. Note that our annotations just reveal variation points, which means that  
