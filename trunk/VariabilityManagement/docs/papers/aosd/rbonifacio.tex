...
\emph{variation in function}, \emph{variation in data}, and \emph{variation in
control flow}~\cite{Bachmann:2001aa}. A reference implementation of the weaving
process (Figure~\ref{fig:weave-process}) is provided in the appendix.
...
Instances of the input models are specified using  \emph{third-part} tools, and
then imported to our variability management environment. We explain which are
these tools and present the Haskell libraries used in our environment.} After
...
The \texttt{checkConfiguration} function traverses both feature model and
product configuration trees, calling specific functions for checking the constraints
applied for each type of feature model relationship (mandatory, alternative,
...
In this MSVCM example, we consider the following scenarios and advices:
...
configuration knowledge. {\color{red}The result is that, in our approach, feature and use
case models are syntactally independent from each other. There is no direct
