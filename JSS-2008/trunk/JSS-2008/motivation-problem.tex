...
When reasoning about modularity in software engineering, very often
the benefit of parallel development is miss-considered. Even after
Parnas has claimed that modularity is more than program structure,
but also process a process structure \cite{}.
...
the use cases related to the complaint management; and another team
is responsible for an auditing concern that must be triggered
whenever changes in a complain occur. In order to enable a parallel
development, at least an agreement about the exposed join-points
must be established.
