...
changes (the set of join-points) and how these join-points should be
...
complaint implementation (row 4) depends on decisions about complaint 
requirements (dependence row 4, column 1), about architectural
...
layer or concern (presentation, distribution, persistence, and so on)} (dependence row
4, column 3), and about the auditing concern (dependence row 4, column 5). 
This last dependence occurs because the team responsible for developing 
...
obeyed in all phases of design and production. Applying design rules 
to our current example, we improve the design structure by removing the 
cyclical dependencies between complaint and auditing concerns. The reviewed 
DSM is presented in Figure~\ref{dsm:hw02}. Notice that a new parameter
(actually a design rule) was introduced (row 4) and all dependences are bellow
the main diagonal (there is no more cyclical dependences). 
...
This idea of defining interfaces between base and aspect code is not really new.
Next, we present some proposal to solve this problem and discuss how our
approach improve existing works.
